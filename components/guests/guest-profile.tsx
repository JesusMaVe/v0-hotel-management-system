...
import { User, MapPin, Calendar, DollarSign, Star, Heart, Utensils, Space as Spa, Car, Wifi } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1">
                ...
                <span className="text-sm text-muted-foreground">{guest.city}</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center p-3 rounded-lg bg-muted">
            <p className="text-lg font-bold text-foreground">${guest.totalSpent.toLocaleString()}</p>
            <p className="text-sm text-muted-foreground">Gasto Total</p>
          </div>
          <div className="text-center p-3 rounded-lg bg-muted">
            <p className="text-lg font-bold text-foreground">{guest.averageStay}</p>
            <p className="text-sm text-muted-foreground">Días Promedio</p>
          </div>
          <div className="text-center p-3 rounded-lg bg-muted">
            <div className="flex items-center justify-center space-x-1 mb-1">
              <Star className="h-4 w-4 text-yellow-500 fill-current" />
              <p className="text-lg font-bold text-foreground">{guest.satisfactionScore}/5</p>
            </div>
      ...
      {/* AI Insights */}
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-foreground mb-4">Insights de IA</h3>

        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
            <div>
              <p className="font-medium text-foreground">Patrón de Gasto</p>
              <p className="text-sm text-muted-foreground">{guest.aiInsights.spendingPattern}</p>
            </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{guest.aiInsights.preferredSeason}</p>
            </div>
          ...
          <div className="p-3 rounded-lg bg-muted">
            <div className="flex items-center justify-between mb-2">
              <p className="font-medium text-foreground">Probabilidad de Upselling</p>
              <span className="text-sm font-medium text-accent">{guest.aiInsights.upsellProbability}%</span>
            </div>
            <Progress value={guest.aiInsights.upsellProbability} className="h-2" />
          </div>

          <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
            <div>
              <p className="font-medium text-foreground">Riesgo de Abandono</p>
              <Badge className={getRiskColor(guest.aiInsights.churnRisk)} variant="outline">
          ...
          <div className="p-3 rounded-lg bg-accent/10 border border-accent/20">
            ...
            <p className="text-sm text-muted-foreground mt-1">{guest.aiInsights.nextVisitPrediction}</p>
          </div>
        </div>
      </Card>
      ...
      {/* Preferences */}
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-foreground mb-4">Preferencias del Huésped</h3>
        ...
        <div className="space-y-4">
          ...
          <div>
            <p className="font-medium text-foreground mb-2">Servicios Favoritos</p>
            <div className="flex flex-wrap gap-2">
          ...
          <div>
            <p className="font-medium text-foreground mb-2">Tipo de Habitación Preferida</p>
