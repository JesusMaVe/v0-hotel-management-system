...
export function CompetitorAnalysis() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-foreground mb-4">Posición en el Mercado</h3>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="p-4 rounded-lg bg-accent/10 border border-accent/20">
            <p className="text-sm text-muted-foreground mb-1">Villa Magna</p>
            <p className="text-xl font-bold text-accent">${ourHotel.averageRate}</p>
            <p className="text-sm text-muted-foreground">{ourHotel.occupancy}% ocupación</p>
          </div>

          <div className="p-4 rounded-lg bg-muted">
            <p className="text-sm text-muted-foreground mb-1">Promedio Mercado</p>
            <p className="text-xl font-bold text-foreground">
              ${Math.round(mockCompetitors.reduce((sum, c) => sum + c.averageRate, 0) / mockCompetitors.length)}
            </p>
            <p className="text-sm text-muted-foreground">
              {Math.round(mockCompetitors.reduce((sum, c) => sum + c.occupancy, 0) / mockCompetitors.length)}% ocupación
            </p>
          </div>

          <div className="p-4 rounded-lg bg-muted">
            <p className="text-sm text-muted-foreground mb-1">Participación</p>
            <p className="text-xl font-bold text-foreground">{ourHotel.marketShare}%</p>
        ...
        <div className="space-y-3">
          <p className="font-medium text-foreground">Participación de Mercado</p>
          {[ourHotel, ...mockCompetitors.sort((a, b) => b.marketShare - a.marketShare)].map((hotel, index) => (
            <div key={index} className="flex items-center space-x-3">
              <div className="w-32 text-sm font-medium text-foreground truncate">{hotel.name}</div>
              <div className="flex-1">
                <Progress value={hotel.marketShare} className="h-3" />
              </div>
              <div className="w-12 text-sm font-medium text-foreground text-right">{hotel.marketShare}%</div>
            </div>
          ))}
        </div>
      </Card>
          ...
          <Card key={competitor.id} className="p-6">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="text-center p-2 rounded bg-muted">
                ...
                <p className="font-medium text-foreground">${competitor.rooms.standard}</p>
              </div>
              <div className="text-center p-2 rounded bg-muted">
                <p className="text-xs text-muted-foreground">Suite</p>
                <p className="font-medium text-foreground">${competitor.rooms.suite}</p>
              </div>
              <div className="text-center p-2 rounded bg-muted">
                <p className="text-xs text-muted-foreground">Premium</p>
                <p className="font-medium text-foreground">${competitor.rooms.premium}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <Progress value={competitor.occupancy} className="flex-1 h-2" />
                  <span className="text-sm font-medium text-foreground">{competitor.occupancy}%</span>
                </div>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Cambio de Precio</p>
                <div className={`flex items-center space-x-1 ${getChangeColor(competitor.priceChange)}`}>
            ...
            <div className="space-y-3">
              <div>
                <p className="text-sm font-medium text-foreground mb-2">Fortalezas</p>
              ...
              <div>
                <p className="text-sm font-medium text-foreground mb-2">Debilidades</p>
                <div className="flex flex-wrap gap-1">
                  {competitor.weaknesses.map((weakness, index) => (
                    <Badge key={index} variant="outline" className="bg-error/10 text-error border-error/20 text-xs">
                      {weakness}
                    </Badge>
                  ))}
                </div>
