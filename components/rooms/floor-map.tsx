  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-foreground">Mapa del Piso {selectedFloor}</h3>
      ...
      <div className="relative bg-muted/20 rounded-lg p-8 min-h-[500px] overflow-hidden">
        ...
        {floorRooms.map((room) => (
          ...
          >
            ...
            {room.guest && (
              <span className="text-xs opacity-80 truncate max-w-full px-1">{room.guest.split(" ")[0]}</span>
            )}
          </button>
        ))}
      </div>
